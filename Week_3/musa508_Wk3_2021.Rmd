...
census_api_key("10633c9166fb86d7953da91f4ad8d215d0a432be", overwrite = TRUE)
```
...
```{r first_maps}
ggplot()+
  geom_sf(data = buffersAndTowns, aes(fill = MUNI))+
  mapTheme()
```
...
```{r first_maps}
ggplot()+
  geom_sf(data = outsideBuffer, aes(fill = Legend))+
  geom_sf(data = insideBuffer, aes(fill = Legend))+
  geom_sf(data = studyAreaTowns, aes(fill = MUNI))+
  scale_fill_viridis_c() +
  geom_sf(data = uga, fill = "transparent")+
  labs(title = "1/8mi Buffer Inside & Outside UGA by Municipality") +
